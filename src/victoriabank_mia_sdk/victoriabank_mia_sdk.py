...
class VictoriabankMiaSdk:
    ...
    def _process_response(self, response: httpx.Response):
        if response.is_error:
            logger.error('%s Error: %d %s', self.__class__.__qualname__, response.status_code, response.text, extra={'method': response.request.method, 'url': response.request.url, 'params': response.request.url.params, 'response_text': response.text, 'status_code': response.status_code})
            #response.raise_for_status()

        if not response.content:
            logger.debug('%s Response: %d', self.__class__.__qualname__, response.status_code, extra={'response_content': response.content})
            return {}
    ...
    def send_request(self, method: str, url: str, form_data: dict = None, json_data: dict = None, params: dict = None, token: str = None, entity_id: str = None):
        ...
        logger.debug('%s Request: %s %s', self.__class__.__qualname__, method, url, extra={'method': method, 'url': url, 'form_data': form_data, 'json_data': json_data, 'params': params, 'token': token})
